# 🧭 Guide Report — Field Service Intelligence Platform

_Generated at **2026-04-16T02:59:41.216610Z** by the factory guide report generator (deterministic, no LLM). Regenerate any time by re-running `scripts/generate_guide_report.py` or by clicking **🧭 Guide Me → Refresh**._

## Snapshot

- **Project slug:** `field-service-intelligence-platform-20260415212450`
- **Status (manifest):** `complete`
- **Network tier:** `public`  **Observability:** `False`
- **Findings:** 🔴 5 critical · 🟠 2 warning · 🟡 1 advisory · ✅ 1 ok

## 🔴 Critical

### Network tier is public but BRD requires private networking

The BRD mentions private endpoints / no public exposure, but `generation_options.networkTier` is `public`. Regenerate with `networkTier: private` so the starter Bicep includes VNet, private endpoints, and NSGs.
...
`C:\Users\fbabaei\app\field-service-intelligence-platform-20260415212450` was created by the starter runner. It will drift from the workspace copy after regeneration. Treat the workspace project as the source of truth.

Related: `C:\Users\fbabaei\app\field-service-intelligence-platform-20260415212450`
...
1. **Run agent `azure-architecture-implementer`** — Regenerate infra + services so they match the BRD and diagram.
   - Arguments: `project-path: projects/field-service-intelligence-platform-20260415212450`, `networkTier: private`, `enableObservability: True`
2. **Run agent `bicep-infrastructure-validator`** — Catch Bicep syntax/logic errors before deploy.
   - Arguments: `project-path: projects/field-service-intelligence-platform-20260415212450`
