...
def _load_stage_module(stage_folder: str, module_alias: str):
    stage_path = Path(__file__).parent.parent / "src" / stage_folder / "main.py"
    spec = importlib.util.spec_from_file_location(module_alias, stage_path)
...
class TestBronzeIngestion(unittest.TestCase):
    ...
    def test_bronze_writes_to_output(self):
        bronze = _load_stage_module("bronze-ingestion", "fabric_bronze_main")
...
class TestSilverProcessor(unittest.TestCase):
    ...
    def setUp(self):
        self.silver = _load_stage_module("silver-processor", "fabric_silver_main")
...
class TestGoldAggregator(unittest.TestCase):
    ...
    def setUp(self):
        self.gold = _load_stage_module("gold-aggregator", "fabric_gold_main")
