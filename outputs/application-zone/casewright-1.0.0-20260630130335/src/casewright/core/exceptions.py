"""Domain exception types for the Casewright API.

These map to HTTP problem responses via the registered exception handlers
in ``casewright.api.exception_handlers``.  Raise these from service /
repository code; the exception-handler layer translates them to the
appropriate HTTP status code and RFC 7807 Problem Details body.
"""
...
class ValidationException(Exception):
    ...
    Args:
        ...
        errors:  Mapping of field name -> list of error strings, mirroring the
                 ...
                 same validation Problem Details serialiser can be reused.
    """
