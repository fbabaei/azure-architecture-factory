"""Centralized exception handlers for the Casewright API.
...
All handlers attach the current OpenTelemetry trace ID (when available) and
a UTC timestamp so callers can correlate errors with distributed traces.

Registration
------------
Call ``register_exception_handlers(app)`` once after the ``FastAPI`` instance
is created::
...
from casewright.core.exceptions import (
    BadRequestException,
    ConflictException,
    NotFoundException,
...
async def _handle_request_validation_error(
    ...
    for error in exc.errors():
        # loc is a tuple like ("body", "field_name") -- use the last segments as the key
...
def register_exception_handlers(app: FastAPI) -> None:
    ...
    Call this once after the ``FastAPI`` instance is created, before any
    middleware or routers are added.
    """
